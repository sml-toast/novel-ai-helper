 ...
 *
 * 拆分原因：26 个任务的示例模板占 provider 近 1/4 篇幅，且只在「没有真实模型」
 * 时才被用到；与调用链路混在一起会让 provider 单文件突破 300 行上限（F089）。
 *
 * 语义原则（F075 A4）：mock 结果**必须**自带「这不是模型输出」的提示，
 * 否则内置示例数据会被误当成 AI 的真实建议。
 *
 * @module server/novel-ai-mock.js
 */

/** 26 类 AI 任务的内置示例：[标题, 正文, tone?] */
const taskTemplates = {
  sync: [
 ...
 * @param {string|null} apiKeyError 密钥解密失败原因；非空时只返回这一条错误卡
 * @returns {Array<{title:string, body:string, tone:string}>}
 */
...
export { buildMockItems, MOCK_NOTICE, taskTemplates };
