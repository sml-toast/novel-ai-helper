...
|---|---|
| 文档日期 | 2026-09-05 |
| 测试基线 | 46 条 Playwright 用例（契约 33 + 页面 13），T015 后全绿 |
...
| 28 | 引用清单 | 提及实体入榜且按分排序、理由可读；prompt 分层不再整包灌上下文 |
| 29 | 正文截断 | 3600 字章节截断至 1800、truncated 元数据回传、prompt 短于原文 |
| 30 | 噪声底限 | 无提及无关键词的实体不进引用清单（全新项目隔离验证） |
| 31 | SSE 流程 | meta→delta→done、首帧 < 500ms、拼接与终态一致、完成落库（唯一标记断言） |
| 32 | 客户端中断 | 服务保持健康；中断的流不写入历史 |
| 33 | 双通道并存 | JSON 通道 /ai 行为不变 |
