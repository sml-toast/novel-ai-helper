...
async function refreshDashboard() {
  ...
  if (!container) return;
  if (!apiOnline) {
    container.innerHTML = renderStatCards({ chapterCount: state.chapters.length, knowledgeCount: 6, aiTaskCount: 0, publishWaiting: state.publishTasks.length, foreshadowOverdue: 0, relationCount: state.relations.length });
...
async function refreshForeshadows() {
  ...
  try {
    ...
    state.foreshadows = result.foreshadows || [];
    renderForeshadowPanel(state.foreshadows);
  } catch (error) {
...
async function addForeshadowFromForm() {
  const title = document.querySelector('#foreshadowTitleInput').value.trim();
  if (!apiOnline) return flashAssist('伏笔登记', 'API 未启动，无法登记伏笔。', 'warning');
  if (!title) return flashAssist('伏笔登记', '请先填写伏笔名称。', 'warning');
  if (!activeChapter) return flashAssist('伏笔登记', '请先选择一个章节作为埋设章节。', 'warning');
  try {
    ...
    flashAssist('伏笔已登记', `${title} · 埋设于《${activeChapter.title}》。`);
    refreshForeshadows();
    refreshDashboard();
  } catch (error) {
...
async function registerForeshadowFromSelection() {
  if (!apiOnline) return flashAssist('伏笔登记', 'API 未启动，无法登记伏笔。', 'warning');
  if (!activeChapter) return flashAssist('伏笔登记', '请先选择一个章节。', 'warning');
  const selection = getEditorSelection();
  ...
  try {
    ...
    flashAssist('伏笔已登记', `「${text.slice(0, 20)}${text.length > 20 ? '…' : ''}」· 埋设于《${activeChapter.title}》。`);
    refreshDashboard();
  } catch (error) {
    flashAssist('伏笔登记失败', error.message, 'danger');
  }
...
async function handleForeshadowAction(foreshadowId, action) {
  if (!apiOnline) return flashAssist('伏笔操作', 'API 未启动，无法更新伏笔状态。', 'warning');
  try {
...
async function loadForeshadowHints() {
...
function renderForeshadowAlerts(foreshadows) {
